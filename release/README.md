...
| `R.S-EXE-Combined-Setup.exe` | Combined single executable — choose Install or Portable at runtime | ~9.5 MB |
...
3. **Combined single EXE (all-in-one):** Run `R.S-EXE-Combined-Setup.exe` to choose between Install (runs the NSIS installer) or Portable (copies the standalone exe to the Desktop).
